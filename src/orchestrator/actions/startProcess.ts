...
import { Action, NodeResult, Workflow, Node } from '@orchestrator/types'
...
export async function startProcess(
  ...
  const workflow = (await orchestrator.redis.get(
  ...
  if (startNode) {
    ...
    if (!isValid) {
      orchestrator.saveResultToProcess(
        { workflow_name, process_id: action.process_id },
        forbiddenState
      )
    ...
    orchestrator.saveResultToProcess(
      { workflow_name, process_id: action.process_id, bag: { ...input } },
      nodeResult
    )
    orchestrator.emitProcessState(actor.id, {
      process_id: action.process_id,
